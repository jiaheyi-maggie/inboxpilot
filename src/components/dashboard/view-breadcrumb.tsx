...
export function ViewBreadcrumb({
  ...
  const showCategoryChip = !!selectedCategory && !isDetailView;
  const showSystemGroupChip = !!selectedSystemGroup && !isDetailView;
  ...
  return (
    <div className="px-4 py-1.5 border-b border-border bg-muted/30 text-sm flex-shrink-0">
      ...
      {/* Chip row: category or system group (only when not in detail view) */}
      {(showCategoryChip || showSystemGroupChip) && (
